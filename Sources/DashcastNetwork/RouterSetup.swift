...
enum RouterSetup {
    ...
    static func script(macLANAddress rawAddress: String,
        ...
        #      reach https://\(hostname) through the router. Tesla's browser refuses
        #      private IPs, so Dashcast serves from \(serviceAddress) on the Mac's loopback.
        #   2. Stops dnsmasq's DNS-rebind protection from dropping answers for \(rebindDomain).
        ...
        # Allow local-looking answers for \(rebindDomain) only (protection stays on for
        # everything else). del_list first so re-running doesn't add a duplicate entry.
        uci -q del_list dhcp.@dnsmasq[0].rebind_domain='\(rebindDomain)' || true
        uci add_list dhcp.@dnsmasq[0].rebind_domain='\(rebindDomain)'
