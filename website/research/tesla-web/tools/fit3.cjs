...
(async () => {
  ...
  for (const [w, h] of sizes) {
    ...
    const r = await p.evaluate(() => {
      ...
      const small = [...document.querySelectorAll('#features *, #touch *, #compare *, #faq *, #office *, .band *, .closing *')].filter((e) => e.children.length === 0 && e.textContent.trim() && e.getBoundingClientRect().width && parseFloat(getComputedStyle(e).fontSize) < 11).map((e) => e.textContent.trim().slice(0, 20));
      const taps = [...document.querySelectorAll('#features a, #features button, #touch button, #compare a, #faq button, .nav a, .nav button, .closing a')].filter((e) => { const rr = e.getBoundingClientRect(); return rr.width && getComputedStyle(e).display !== 'none' && (rr.height < 40 || rr.width < 40); }).map((e) => (e.textContent.trim() || e.getAttribute('aria-label')).slice(0, 16) + ' ' + Math.round(e.getBoundingClientRect().width) + 'x' + Math.round(e.getBoundingClientRect().height));
