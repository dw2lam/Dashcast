...
async function run(view, label) {
  ...
  const center = async () => {
    await p.evaluate(() => { const t = document.getElementById('touch-cards'); window.scrollTo(0, t.getBoundingClientRect().top + scrollY - 200); });
    await sleep(400);
    return p.evaluate(() => { const r = document.getElementById('touch-cards').getBoundingClientRect(); return { x: r.left + r.width / 2, y: r.top + r.height / 2 }; });
  };
  const reset = async () => { await p.evaluate(() => { document.getElementById('touch-cards').scrollLeft = 0; }); await sleep(300); };
  const state = () => p.evaluate(() => { const t = document.getElementById('touch-cards'); const cards = [...t.querySelectorAll('.card')]; const f = cards[0].offsetLeft; const snaps = cards.map((c) => Math.min(t.scrollWidth - t.clientWidth, c.offsetLeft - f)); return { left: Math.round(t.scrollLeft), snapped: snaps.some((s) => Math.abs(s - t.scrollLeft) < 2), pageY: Math.round(scrollY) }; });
  ...
  if (!view.hasTouch) {
    ...
    await p.evaluate(() => { window.__clicks = 0; document.getElementById('touch-cards').addEventListener('click', () => window.__clicks++); });
    ...
    box = await center(); await reset(); await p.evaluate(() => document.getElementById('touch-cards').focus()); a = await state();
    await p.keyboard.press('ArrowRight'); await sleep(900); z = await state(); log('ArrowRight', a, z);
    a = z; await p.keyboard.press('End'); await sleep(1000); z = await state(); log('End', a, z, `next disabled:${await p.evaluate(() => document.querySelector('.cards__nav--next').disabled)}`);
    a = z; await p.keyboard.press('Home'); await sleep(1000); z = await state(); log('Home', a, z, `prev disabled:${await p.evaluate(() => document.querySelector('.cards__nav--prev').disabled)}`);
  } else {
...
(async () => {
  await run({ width: 1512, height: 945 }, 'desktop');
  await run({ width: 390, height: 844, isMobile: true, hasTouch: true, deviceScaleFactor: 2 }, 'phone');
})();
